...
-- SUPPRIMÉ: t1.value3:ShowCredit() -- Enlève le crédit RayVoid au démarrage
t1.value3:SetScriptName("AKkiwi Skin Changer")
...
print("[+] Initializing AKkiwi Skin Changer v2.2 (HEURISTIC SCAN Edition)...")
_G.AKkiwiVersion = "2.2-GC"
...
function t1.value12(p9, p10)
    if t1.value5[p10] then
        warn("[AKkiwi] Refusing to equip '" .. p10 .. "': skin is blacklisted (known to cause a softlock).")
...
function t1.value14(p13, p14)
    if t1.value6[p14] then
        warn("[AKkiwi] Refusing to equip charm '" .. p14 .. "': blacklisted (known to crash).")
...
-- Variable pour stocker la fonction de rendu pour qu'elle soit accessible
local renderTabFunction
...
task.spawn(function()
    ...
    for k in pairs(v64) do
        ...
        if not _G.EquippedData[v70] then
            ...
            if t1.value5[_G.EquippedData[v70].Skin] then
                warn("[AKkiwi] '" .. _G.EquippedData[v70].Skin .. "' was equipped on " .. v70 .. " but is blacklisted -> reset to Default.")
                _G.EquippedData[v70].Skin = "Default"
            end

            if t1.value6[_G.EquippedData[v70].Charm] then
                warn("[AKkiwi] Charm '" .. _G.EquippedData[v70].Charm .. "' was equipped on " .. v70 .. " but is blacklisted -> reset to None.")
                _G.EquippedData[v70].Charm = "None"
            end
        end
    end
    ...
    function ClientViewModel.new(p18, p19)
        ...
        if success then
            ...
            warn("[AKkiwi] ViewModel construction failed (likely a broken charm):", (tostring(resultValue)))
            pcall(function()
            ...
            if ok then
                u106 = result
                warn("[AKkiwi] Weapon rebuilt WITHOUT the charm after failure. Add this charm to BROKEN_CHARMS.")
            else
                warn("[AKkiwi] Still failed even without the charm -- unrelated issue. Re-raising the original error.")
                error(resultValue, 0)
            end
        end
    ...
    local t9 = {
		Enabled = true,
		FileName = "AKkiwiSkinConfig"
	}
    local v77 = CreateWindow(value3, {
		Name = "AKkiwi Skin Changer",
		LoadingTitle = "AKkiwi Skin Changer",
    ...
    -- Définition de la fonction de rendu avec correction du bug
    renderTabFunction = function(p26)
        ...
        v130:CreateDropdown({
            ...
            if v170 and v170 ~= u80 then
                -- CORRECTION: Appeler renderTabFunction avec la nouvelle arme
                task.delay(0.5, function()
                    renderTabFunction(v170)
                end)
            end
        end
		})
    ...
    -- Appel initial avec la première arme
    renderTabFunction(u80)
    print("[+] AKkiwi Skin Changer loaded. Press K to toggle.")
