...
-- Version corrigée avec exécution réelle
...
function get_game_id()
    return tonumber(game.PlaceId) or 0, tonumber(game.GameId) or 0
end
...
local function execute_game()
    ...
    -- Forcer le chargement du jeu
    -- Le jeu utilise généralement un RemoteEvent ou une fonction dans ReplicatedStorage
    local game_loader = ReplicatedStorage:FindFirstChild("GameLoader")
    if game_loader and game_loader:IsA("RemoteEvent") then
        game_loader:FireServer("LoadGame", {
            PlaceId = config.TARGET_PLACE_ID,
            Key = "san_diego",
            Beta = true
        })
        print("[NetNull] Game loader triggered via RemoteEvent")
    end
    
    -- Alternative: chercher et exécuter le script de chargement
    local loader_script = ReplicatedStorage:FindFirstChild("Loader")
    if loader_script and loader_script:IsA("ModuleScript") then
        local success, result = pcall(function()
            return require(loader_script)
        end)
        if success then
            print("[NetNull] Loader ModuleScript executed")
        end
    end
    
    -- Autre alternative: injecter directement dans le jeu
    local game_module = ReplicatedStorage:FindFirstChild("SharedModules")
    if game_module then
        local main = game_module:FindFirstChild("Main")
        if main and main:IsA("ModuleScript") then
            pcall(function()
                require(main)
            end)
            print("[NetNull] Main ModuleScript executed")
        end
    end
    
    -- Forcer l'activation du mode BETA dans le jeu
    -- Certains jeux utilisent des valeurs dans _G
    getgenv().BETA_MODE = true
    getgenv().NETNULL_BETA = true
    _G.BETA_MODE = true
    _G.NETNULL_BETA = true
    
    print("[NetNull] San Diego Border Roleplay BETA should now be loading...")
    
    -- Si le jeu a un système de chargement personnalisé
    -- On essaie de déclencher les événements de chargement
    local remote_events = ReplicatedStorage:FindFirstChild("__remotes")
    if remote_events then
        for _, child in ipairs(remote_events:GetChildren()) do
            if child:IsA("RemoteEvent") and string.find(child.Name, "Load") then
                pcall(function()
                    child:FireServer("BetaLoad", {
                        mode = "beta",
                        key = "san_diego"
                    })
                end)
    ...
    -- Dernier recours: simuler le chargement via les fonctions du jeu
    if type(getgenv().LoadGame) == "function" then
        getgenv().LoadGame("beta")
    end
...
local function initialize_beta()
    ...
    -- EXECUTER LE JEU
    local success = execute_game()
    
    if success then
        print("[NetNull] Game loading initiated successfully!")
    else
        print("[NetNull] Game loading may have failed, but continuing...")
    end
    ...
    if loader_gui then
        loader_gui:Destroy()
        print("[NetNull] Loader UI removed")
    end
...
if not success then
    ...
    print("[NetNull] Script executed successfully!")
end